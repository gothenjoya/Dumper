# Thanks to the kind soul from the .gg/rbxoffsets server for sending this
...
class FFlagsDumper:
    ...
    def save_csharp(self, fflags, filename="offsets.hpp"):
        fflags_sorted = sorted(fflags, key=lambda x: x[0])
        ...
        with open(filename, "w", encoding="utf-8") as f:
            f.write("#pragma once\n")
            f.write(f"/*  Roblox Version  : {self.version}\n")
            f.write(f"/*  Dumped At       : {now.strftime('%H:%M %d/%m/%Y')} (GMT)\n")
            f.write(f"/*  Total Offsets   : {len(fflags_sorted)}\n")
            f.write("*/\n\n")

            f.write("#include <cstdint>\n")
            f.write("#include <string>\n\n")

            f.write("namespace FFlagOffsets {\n")
            f.write(f'    inline std::string ClientVersion = "{self.version}";\n\n')

            f.write("    namespace FFlagList {\n")
            f.write(f"         inline constexpr uintptr_t Pointer = 0x{self.fflags_ptr_rva:X};\n")
            f.write("         inline constexpr uintptr_t ToFlag = 0x30;\n")
            f.write("         inline constexpr uintptr_t ToValue = 0x0;\n")
            f.write("    }\n\n")
            ...
            for name, rva in fflags_sorted:
                ...
                f.write(
                    f"         inline constexpr uintptr_t {clean_name} = 0x{rva:X};\n"
                )
